...
#        data = bytes(data.encode('ascii'))
        data = re.findall('..',data.hex())
        writecommand = [WRITE,upper,lower]
        for i in data: writecommand.append(int(i,16))
        self.spi.writebytes([WREN])  # Write Enable required before write can take place. Resets after Write
        time.sleep(0.005)

        self.spi.writebytes(writecommand)
        time.sleep(0.005)
    ...
    def status(self):
        values = self.spi.xfer2([RDSR, 0x00])
        print(values)
